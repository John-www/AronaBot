...
TOKEN_ENV = "ARONA_DISCORD_TOKEN"
DUNGEON_GM_MODEL = os.getenv("DUNGEON_GM_MODEL", os.getenv("RAG_OLLAMA_MODEL", "qwen2.5:3b"))
ALLOWED_CHANNEL_ID = os.getenv("DISCORD_CHANNEL_ID", "714882888503656504")
DAILY_WEATHER_CHANNEL_ID = os.getenv("DAILY_WEATHER_CHANNEL_ID", ALLOWED_CHANNEL_ID)
DAILY_WEATHER_HOUR = int(os.getenv("DAILY_WEATHER_HOUR", "6"))
DAILY_WEATHER_MINUTE = int(os.getenv("DAILY_WEATHER_MINUTE", "0"))
TAIPEI_TZ = ZoneInfo("Asia/Taipei")
DAILY_WEATHER_LOCATIONS = ["台北", "新竹", "桃園"]
...
def build_daily_weather_report():
    page_html = fetch_text(CWA_WEEK_URL, timeout=15)
    forecasts = parse_cwa_week_table(page_html)
    try:
        valid_time = parse_cwa_week_time(fetch_text(CWA_WEEK_TIME_URL, timeout=10))
    ...
    for location in DAILY_WEATHER_LOCATIONS:
        county = normalize_county_name(location)
        county_forecast = forecasts.get(county) if county else None
        if not county_forecast:
            lines.append(f"{location}：查無資料")
            continue
        today = county_forecast[0]
        day_part = today["day"] or {"weather": "未知", "temp": "--"}
        night_part = today["night"] or {"weather": "未知", "temp": "--"}
        lines.append(
            f"{county} {today['date']}："
            f"早上 {day_part['weather']} {day_part['temp']} C；"
            f"中午 {day_part['weather']} {day_part['temp']} C；"
            f"晚上 {night_part['weather']} {night_part['temp']} C"
        )
    return "\n".join(lines)


async def get_daily_weather_channel():
    if not DAILY_WEATHER_CHANNEL_ID:
        return None
    channel_id = int(DAILY_WEATHER_CHANNEL_ID)
    return bot.get_channel(channel_id) or await bot.fetch_channel(channel_id)


@tasks.loop(minutes=1)
async def daily_weather_job():
    global DAILY_WEATHER_LAST_DATE
    now = datetime.now(TAIPEI_TZ)
    if now.hour != DAILY_WEATHER_HOUR or now.minute != DAILY_WEATHER_MINUTE:
        return
    if DAILY_WEATHER_LAST_DATE == now.date():
        return

    try:
        channel = await get_daily_weather_channel()
        if not channel:
            print(">> Daily weather skipped: channel not found <<")
            return
        report = await asyncio.to_thread(build_daily_weather_report)
        for part in split_message(report):
            await channel.send(part)
        DAILY_WEATHER_LAST_DATE = now.date()
    except Exception as exc:
        print(f">> Daily weather failed: {exc} <<")


@daily_weather_job.before_loop
async def before_daily_weather_job():
...
async def on_ready():
    ...
    print(f">> ARONA bot is online as {bot.user} <<")
    ...
    if not daily_weather_job.is_running():
        daily_weather_job.start()
        print(
            f">> Daily weather scheduled at {DAILY_WEATHER_HOUR:02d}:{DAILY_WEATHER_MINUTE:02d} "
            f"Asia/Taipei for channel {DAILY_WEATHER_CHANNEL_ID} <<"
        )
...
def main():
    token = os.getenv(TOKEN_ENV) or os.getenv("DISCORD_TOKEN")
    if not token:
